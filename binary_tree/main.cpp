#include<iostream>
using namespace std;
//Nhập vào cây nhị phân các số nguyên. Xuất ra màn hình các phần tử của cây nhị phân
...
void ThemNodeVaoCay(TREE &t, int x) {
	//Nếu cây rỗng
	if (t == NULL) {
		NODE* p = new NODE; //Khởi tạo một node để thêm vào cây
	...
	else //Cây có tồn tại phần tử 
	{
		...
		if (t->data > x) {
			ThemNodeVaoCay(t->pLeft, x); //Duyệt qua bên trái để thêm phần tử x
		}
		else if(t->data < x) //Nếu phần tử thêm vào lơn hơn node gốc ==> Thêm nó vào bên phải
		{
...
//Hàm xuất cây nhị phần theo NLR
